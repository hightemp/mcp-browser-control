...
reconnecting the server. Observe includes website access and `webRequest` for
bounded network-idle waits; it does not expose request bodies or credentials.
Personal data also requests Observe website access for origin-scoped cookie and
storage operations. It includes `sessions` for recently closed entries and
`tabGroups` for group title, color, and collapsed state management.

...
| Trusted root-document input | managed `Input.dispatchMouseEvent`/`dispatchKeyEvent`/`insertText` plus content locator preparation | Observe + Debug | Capability-gated | Capability-gated; release smoke required | Not supported until tested |
| Viewport screenshot | `tabs.captureVisibleTab`, target-origin access, and a user-invoked temporary `activeTab` grant | Observe plus Core | Capability-gated; invoke the extension action per tab/origin | Capability-gated; release smoke required | Capability-gated after explicit action invocation |
| Full-page and element screenshot | managed `Page.getLayoutMetrics`/`Page.captureScreenshot`, root-document locator, target-origin access | Observe + Debug | Capability-gated | Capability-gated; release smoke required | Not supported until tested |

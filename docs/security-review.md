...
| SR-06 | Raw CDP or JavaScript evaluation bypasses tool, origin, permission, or data controls. | Evaluation and reviewed raw CDP use separate disabled-by-default capabilities and dedicated `full` tools; `browser_send_command` cannot bypass them. Debugger permission is optional and user-granted. | Every CDP consumer must use the ref-counted session manager and exact method allowlists. Typed tools must reapply origin, permission, deadline, redaction, result-size, and audit checks. Main-world, persistent, and unrestricted raw evaluation remain prohibited. |
...
| Full-page and element screenshots | Dedicated typed capture modes with root-document checks, fixed CDP methods, bounded artifact output, and no scroll/viewport mutation | `standard` plus Observe/Debug; viewport mode needs no Debug but requires a user-invoked temporary Core `activeTab` grant instead of `<all_urls>` |
...
| T-058 screenshots | Implemented through one typed standard-profile tool with user-invoked temporary `activeTab` access for viewport mode, viewport/full-page/element modes, root tab/origin/document checks, exact managed `Page.getLayoutMetrics`/`Page.captureScreenshot` leases for Debug modes, no scroll or viewport mutation, independent PNG/JPEG dimension and byte validation, artifact-only output, and a sensitive-content warning; see [`screenshots.md`](screenshots.md) |
...
| T-062 network capture | Implemented through six full-profile typed tools with an exact managed Network lease, document-scoped bounded ring/TTL, redirect and failure metadata, redacted headers/URLs, same-origin allowlisted textual body artifacts, body-free bounded HAR export, independent server redaction/validation, metadata-only audit, and generic-command denial; interception, request modification, binary/cross-origin bodies, cache mutation, and raw request IDs remain prohibited; see [`network-capture.md`](network-capture.md) |
...
| T-065 evaluation | Implemented as a typed full-profile root-frame tool with Observe + Debug, a disabled-by-default user feature flag, restrictive isolated-world CSP, exact Page/Runtime methods, fixed execution bounds, JSON-only output, request object-group release, independent server validation, and a generic-command bypass denial; main-world, persistent, and unreviewed raw execution remain prohibited; see [`javascript-evaluation.md`](javascript-evaluation.md) |
| T-066 raw CDP | Implemented as a dedicated full-profile, disabled-by-default root-frame tool for the seven reviewed read-only methods; exact method/parameter/result allowlists, independent extension/server validation, no handles or streams, bounded output, metadata-only audit, batch/generic-command bypass denials, and repeated origin/document checks are enforced; see [`raw-cdp.md`](raw-cdp.md) |
| T-067 performance | Implemented as two dedicated full-profile tools: bounded inline numeric metrics and short-lived trace/coverage/CPU-profile/audit sessions with fixed CDP contracts, artifact-only capture output, metadata-only audit, cleanup on every exit, and batch/generic-command bypass denials; heap snapshots, caller-controlled trace categories, stream handles, and continuous profiling remain prohibited; see [`performance-diagnostics.md`](performance-diagnostics.md) |
| T-070 cookies | Implemented through four dedicated full-profile tools with Personal data + Observe, exact root-origin/document/store checks before and after the API call, paginated metadata, default value masking, a separate disabled-by-default sensitive-data capability, bounded partition metadata, independent extension/server validation, metadata-only audit, and batch/generic-command denials; see [`cookies.md`](cookies.md) |
| T-071 Web Storage | Implemented through seven dedicated full-profile tools with Personal data + Observe, exact root-origin/document checks before and after isolated-world access, default value masking, separately advertised sensitive reads, bounded pagination and inventories, Cache Storage names only, IndexedDB names/versions only, confirmed preflighted clear, metadata-only audit, independent extension/server validation, and batch/generic-command denials; see [`web-storage.md`](web-storage.md) |
| T-072 downloads | Implemented through seven dedicated full-profile tools with Personal data gating, bounded pagination/status, basename-only path metadata, stripped URL secrets, strict lifecycle preconditions, exact single terminal history erase with confirmation, incognito action policy, metadata-only audit, independent extension/server validation, and batch/generic-command denials; file reads/deletion, arbitrary paths/headers/bodies, danger acceptance, and bulk erase remain prohibited; see [`downloads.md`](downloads.md) |
| T-073 history/bookmarks/reading list | Implemented through 14 dedicated full-profile tools with domain-specific optional permissions, 10,000-item scans, 200-item pagination, normalized typed results, credential/fragment/query-secret URL redaction, independent server enforcement, metadata-only audit, confirmation for every history deletion and recursive bookmark removal, permission rechecks, and generic-command/batch denials; Reading List handlers remain capability-gated, but the portable manifest excludes its permission because an accepted unknown API fatally crashes Yandex Browser before runtime detection; see [`personal-data.md`](personal-data.md) |

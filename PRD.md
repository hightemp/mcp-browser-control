...
Portable-манифест не объявляет `readingList`: Yandex Browser принимает этот
optional permission, но затем фатально завершает extension process при создании
bindings неизвестного API до выполнения JavaScript. Реализация Reading List
остаётся capability-gated для будущей browser-specific сборки.

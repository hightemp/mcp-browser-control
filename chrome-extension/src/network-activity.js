...
export function createNetworkActivityObserver(chromeAPI, now = () => Date.now()) {
  ...
  function start() {
    ...
    try {
      webRequest.onBeforeRequest.addListener(onStarted, { urls: ["<all_urls>"] });
      webRequest.onCompleted.addListener(onFinished, { urls: ["<all_urls>"] });
      webRequest.onErrorOccurred.addListener(onFinished, { urls: ["<all_urls>"] });
      started = true;
      return true;
    } catch {
      return false;
  ...
  function waitForIdle({ tabId, idleMs, signal }) {
    ...
    const evaluate = () => {
      ...
      idleTimer = setTimeout(() => finish(() => resolvePromise({
